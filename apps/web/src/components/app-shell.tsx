...
export async function PageHeader({ eyebrow, title, children }: { eyebrow?: string; title: string; children?: React.ReactNode }) {
  ...
  return <div className="topbar"><div className="topbar-title"><BackButton/><div>{visibleEyebrow && <div className="eyebrow">{visibleEyebrow}</div>}<h1>{title}</h1></div></div><div className="topbar-actions">{children}{viewerCanManage(viewer) ? <Link className="event-chip" href="/events#event-list" aria-label="Choose the active event">{eventChip}</Link> : <div className="event-chip">{eventChip}</div>}</div></div>;
}
