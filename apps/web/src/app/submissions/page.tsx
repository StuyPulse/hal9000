...
export default async function SubmissionsPage({ searchParams }: { searchParams: Promise<{ scope?: string; match?: string }> }) {
  ...
  let query = (supabase as any).from("scouting_entries").select("id,entry_type,status,submitted_at,created_at,matches(match_number,match_type,tba_match_key),teams(team_number,name),profiles(display_name)").order("created_at", { ascending: false }).limit(100);
