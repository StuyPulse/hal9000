...
export default async function SubmissionDetailPage({ params }: { params: Promise<{ entryId: string }> }) {
  ...
  return <AppShell active="Submissions"><PageHeader title={teamName}><Link className="link" href="/submissions">All submissions</Link></PageHeader><section className="card submission-detail"><div className="submission-detail-head"><div><span className="submission-kind">{entry.entry_type.replace("_", " ")} report</span><h2>{entry.entry_type === "match" && entry.matches?.match_number ? `Qualification ${entry.matches.match_number}` : "Scouting report"}</h2><p className="muted">{entry.profiles?.display_name ?? "Scout"} · {timestamp ? <LocalDateTime value={timestamp}/> : "Saved draft"}</p></div><span className={`tag ${entry.status === "submitted" ? "complete" : "pending"}`}>{entry.status}</span></div><div className="submission-meta"><span>Updated <LocalDateTime value={entry.updated_at}/></span></div><PayloadGrid payload={entry.payload ?? {}} teamNames={teamNames}/></section></AppShell>;
}
