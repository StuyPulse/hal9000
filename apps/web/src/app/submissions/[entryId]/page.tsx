...
export default async function SubmissionDetailPage({ params, searchParams }: { params: Promise<{ entryId: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  ...
  return <AppShell active="Submissions"><PageHeader title={teamName}><Link className="link" href={submissionsHref}>All submissions</Link></PageHeader><section className="card submission-detail"><div className="submission-detail-head"><div><span className="submission-kind">{scoutingEntryTypeLabel(entry.entry_type)} report</span><h2>{scoutingEntryLabel(entry)}</h2><p className="muted">Author: {entry.author?.display_name ?? "Scout"} · {timestamp ? <LocalDateTime value={timestamp}/> : "Saved draft"}</p></div><div className="row-actions">{editHref && <Link className="button secondary" href={editHref}>Edit report</Link>}{canDelete && <DeleteSubmissionForm entryId={entry.id} compact/>}<span className={`tag ${entry.status === "submitted" ? "complete" : "pending"}`}>{entry.status}</span></div></div><div className="submission-meta"><span>Updated <LocalDateTime value={entry.updated_at}/>{entry.editor && <span> · Last edited by {entry.editor.display_name ?? "Scout"}</span>}</span></div><PayloadGrid payload={entry.payload ?? {}} teamNames={teamNames}/></section></AppShell>;
}
