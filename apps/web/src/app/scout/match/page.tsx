...
export default async function MatchScoutPage({ searchParams }: { searchParams: Promise<{ match?: string }> }) {
  ...
  const [{ data: matches }, { data: eventTeams }] = event ? await Promise.all([
    supabase.from("matches").select("id,tba_match_key,match_number,match_type,status,updated_at,red_teams,blue_teams").eq("event_id", event.id).order("scheduled_at"),
  ...
  return <AppShell active="Manual scouting">
    ...
    {event ? <div className="match-scout-grid">
      <MatchScoutPicker matches={(matches ?? []).map((match) => ({ id: match.id, key: match.tba_match_key, number: match.match_number, type: match.match_type, status: match.status, updatedAt: match.updated_at, red: match.red_teams, blue: match.blue_teams }))} teams={teams} initialMatchId={(matches ?? []).some((match) => match.id === requestedMatchId) ? requestedMatchId : ""} />
