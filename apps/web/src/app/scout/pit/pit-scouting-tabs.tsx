...
export function PitScoutingTabs({ eventId, organizationId, scoutUserId, teams, missing, photographed, scoutedTeamIds, editingEntryId, initialTeamId, initialPayload, returnTo }: { eventId: string; organizationId: string; scoutUserId: string; teams: Team[]; missing: Team[]; photographed: number; scoutedTeamIds: string[]; editingEntryId?: string; initialTeamId?: string; initialPayload?: Record<string, unknown>; returnTo?: string }) {
  ...
  const tabs: { id: Tab; label: string }[] = [{ id: "scouting", label: "Pit form" }, { id: "coverage", label: "Photos & coverage" }];
  ...
  return <section className="pit-tabs-workspace">
    ...
    <div id="pit-panel-scouting" role="tabpanel" aria-labelledby="pit-tab-scouting" hidden={activeTab !== "scouting"}><ManualScouting eventId={eventId} organizationId={organizationId} scoutUserId={scoutUserId} teams={teams} type="pit" markedTeamIds={scoutedTeamIds} editingEntryId={editingEntryId} initialTeamId={initialTeamId} initialPayload={initialPayload} returnTo={returnTo}/></div>
    <div id="pit-panel-coverage" role="tabpanel" aria-labelledby="pit-tab-coverage" hidden={activeTab !== "coverage"}><div className="pit-photo-tab-stack"><PitPhotoUpload eventId={eventId} teams={teams} selectedTeamId={photoTeamId} onSelectedTeamChange={setPhotoTeamId}/><section className="card pit-photo-coverage"><div className="card-head"><div><h2>Pit-photo coverage</h2><p className="muted">{photographed} of {teams.length} teams have at least one pit photo.</p></div>{missing.length > 0 && <span className="tag pending">{missing.length} remaining</span>}</div>{missing.length ? <div className="pit-photo-missing"><p>Teams still needing a pit photo</p><div className="pit-photo-missing-list">{missing.map((team) => <button type="button" key={team.id} onClick={() => showPhotoUpload(team.id)}><strong>{team.number}</strong><small>{team.name}</small></button>)}</div></div> : <div className="pit-photo-complete" role="status"><span aria-hidden="true">✓</span><div><strong>Every team has a pit photo.</strong><p>Coverage complete — great work, PulseCrew.</p></div></div>}</section></div></div>
  </section>;
}
