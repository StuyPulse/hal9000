...
export default async function MatchStrategyPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const [{ data: matches }, { data: eventTeams }, { data: entries }] = await Promise.all([
    supabase.from("matches").select("id,match_number,match_type,scheduled_at,status,red_teams,blue_teams").eq("event_id", event.id).order("scheduled_at", { nullsFirst: false }).order("match_number"),
  ...
  const scheduledMatches = (matches ?? []).map((match: any) => ({ id: match.id, number: match.match_number, type: match.match_type, scheduledAt: match.scheduled_at, status: match.status, red: match.red_teams ?? [], blue: match.blue_teams ?? [] }));
