...
export default async function MatchStrategyPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const teams = (eventTeams ?? []).map((link: any) => ({ id: link.team_id, number: link.teams?.team_number ?? 0, name: link.teams?.name ?? "Unknown team", stats: calculateScoutStats(competitiveMatchEntries((entries ?? []).filter((entry: any) => entry.team_id === link.team_id))) })).sort((left, right) => left.number - right.number);
