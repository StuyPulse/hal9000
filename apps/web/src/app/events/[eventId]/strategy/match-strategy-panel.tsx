...
const matchSearchLabel = (match: Match) => `${matchLabel(match)} · ${match.status}${match.scheduledAt ? ` · ${new Date(match.scheduledAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}` : ""}`;

function SearchableMatchSelect({ matches, value, onValueChange }: { matches: Match[]; value: string; onValueChange: (value: string) => void }) {
  const [query, setQuery] = useState("");
  const [open, setOpen] = useState(false);
  const listId = useId();
  const selected = matches.find((match) => match.id === value);
  const results = matches.filter((match) => matchSearchLabel(match).toLowerCase().includes(query.trim().toLowerCase()));
  ...
  return <div className="searchable-team-select strategy-match-select" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) close(); }}>
    <div className="searchable-team-input">
      <input id="strategy-match" value={open ? query : selected ? matchSearchLabel(selected) : ""} disabled={!matches.length} role="combobox" aria-autocomplete="list" aria-expanded={open} aria-controls={listId} placeholder={matches.length ? "Search matches…" : "No matches in this event"} autoComplete="off" onFocus={() => { setQuery(""); setOpen(true); }} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onKeyDown={(event) => { if (event.key === "Escape") close(); if (event.key === "ArrowDown") setOpen(true); if (event.key === "Enter" && results[0]) { event.preventDefault(); choose(results[0]); } }}/>
    ...
    {open && <div id={listId} className="searchable-team-results" role="listbox" aria-label="Matches">{results.length ? results.map((match) => <button key={match.id} type="button" role="option" aria-selected={match.id === value} className={match.id === value ? "selected" : ""} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(match)}><strong>{matchLabel(match)}</strong><span>{match.status}{match.scheduledAt ? ` · ${new Date(match.scheduledAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}` : ""}</span></button>) : <p className="muted">No matches match that search.</p>}</div>}
  </div>;
}
