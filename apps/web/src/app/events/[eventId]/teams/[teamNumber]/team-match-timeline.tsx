...
const breakageDetails = (payload: Record<string, unknown>) => {
  if (payload.robot_broke !== true) return "No";
  const issues = Array.isArray(payload.breakage_issues) ? payload.breakage_issues : payload.break_tag ? [{ issue: payload.break_tag, timestamp: payload.break_timestamp }] : [];
  const details = issues.map((item) => {
    const issue: Record<string, unknown> = item && typeof item === "object" ? item as Record<string, unknown> : { issue: item };
    const label = typeof issue.issue === "string" && issue.issue.trim() ? issue.issue.trim() : "Issue not recorded";
    return issue.timestamp ? `${label} (${String(issue.timestamp)})` : label;
  });
  return details.length ? details.join(" · ") : "Yes";
};
