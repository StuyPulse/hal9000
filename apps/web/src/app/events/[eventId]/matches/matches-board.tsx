...
function ManualMatchSetup({ eventId, teams, matches }: { eventId: string; teams: Team[]; matches: Match[] }) {
  ...
  return <section className="manual-event-section">
    ...
    <form key={editing?.id ?? "new"} action={action} onSubmit={captureScheduledAtInstant} className="manual-match-form">
      ...
      <label><span>Round</span><select name="matchType" defaultValue={editing?.type ?? "qualification"} onChange={(event) => { if (!editing) { const numberInput = event.currentTarget.form?.elements.namedItem("matchNumber") as HTMLInputElement | null; if (numberInput) numberInput.value = String(nextManualMatchNumber(matches, event.currentTarget.value)); } }}><option value="qualification">Qualification</option><option value="playoff">Playoff</option><option value="practice">Practice</option></select></label>
