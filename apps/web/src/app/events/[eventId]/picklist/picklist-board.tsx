...
function TierTeamRow({ team, ranking, categories, tags, tierIndex, tierSize, canEdit, saving, dragging, dropTarget, onDragStart, onDragEnd, onDragOver, onDrop, onTierChange, onMove, onNote, onSelected, onTags }: { team: Team; ranking?: Ranking; categories: Category[]; tags: PicklistTag[]; tierIndex: number; tierSize: number; canEdit: boolean; saving: boolean; dragging: boolean; dropTarget: boolean; onDragStart: (event: DragEvent<HTMLElement>) => void; onDragEnd: () => void; onDragOver: (event: DragEvent<HTMLElement>) => void; onDrop: (event: DragEvent<HTMLElement>) => void; onTierChange: (categoryId: string) => void; onMove: (direction: -1 | 1) => void; onNote: (note: string) => void; onSelected: (selected: boolean) => void; onTags: (tagIds: string[]) => void }) {
  const [note, setNote] = useState(ranking?.note ?? ""); const [tagPickerOpen, setTagPickerOpen] = useState(false); const tagPickerRef = useRef<HTMLDivElement>(null); const currentTier = ranking?.category_id ?? UNSORTED_TIER_ID; const selected = ranking?.selected ?? false; const tagIds = ranking?.tag_ids ?? []; const activeTags = tags.filter((tag) => tagIds.includes(tag.id));
  useEffect(() => {
    if (!tagPickerOpen) return;
    const closeWhenOutside = (event: PointerEvent) => { if (!tagPickerRef.current?.contains(event.target as Node)) setTagPickerOpen(false); };
    document.addEventListener("pointerdown", closeWhenOutside);
    return () => document.removeEventListener("pointerdown", closeWhenOutside);
  }, [tagPickerOpen]);
  const teamIdentity = <div className="picklist-team-name">{canEdit && <span className="picklist-drag-source" draggable onDragStart={(event) => onDragStart(event as unknown as DragEvent<HTMLElement>)} onDragEnd={onDragEnd} aria-label={`Drag Team ${team.team_number} to reorder`}><GripVertical className="picklist-drag-handle" size={18} aria-hidden="true"/></span>}<span className="picklist-team-position">{tierIndex + 1}</span><span><strong>{team.team_number}{team.eventRank ? <em className="picklist-event-rank">#{team.eventRank}</em> : null}</strong><small>{team.name}</small></span></div>;
  const tagControl = canEdit ? <div className="picklist-tag-picker" ref={tagPickerRef}><button type="button" className={tagIds.length ? "picklist-tag-picker-trigger has-tags" : "picklist-tag-picker-trigger"} disabled={saving} aria-label={`Manage tags and tier for Team ${team.team_number}${activeTags.length ? `; tags: ${activeTags.map((tag) => tag.name).join(", ")}` : ""}`} aria-expanded={tagPickerOpen} aria-controls={`picklist-team-tags-${team.id}`} onClick={() => setTagPickerOpen((open) => !open)}><TagIcon size={15} aria-hidden="true"/>{activeTags.map((tag) => <span key={tag.id} className="picklist-tag-dot" style={{ "--tag": tag.color } as CSSProperties} aria-hidden="true"/>)}</button>{tagPickerOpen && <div id={`picklist-team-tags-${team.id}`} className="picklist-tag-picker-menu" aria-label={`Tags and tier for Team ${team.team_number}`}>{tags.map((tag) => <button key={tag.id} type="button" disabled={saving} style={{ "--tag": tag.color } as CSSProperties} className={tagIds.includes(tag.id) ? "active" : ""} aria-pressed={tagIds.includes(tag.id)} onClick={() => onTags(tagIds.includes(tag.id) ? tagIds.filter((id) => id !== tag.id) : [...tagIds, tag.id])}>{tag.name}</button>)}<label className="picklist-picker-tier-select"><span>Tier</span><select value={currentTier} disabled={saving} onChange={(event) => { onTierChange(event.target.value); setTagPickerOpen(false); }}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label></div>}</div> : <div className="picklist-team-tags read-only">{activeTags.map((tag) => <span key={tag.id} style={{ "--tag": tag.color } as CSSProperties}>{tag.name}</span>)}</div>;
