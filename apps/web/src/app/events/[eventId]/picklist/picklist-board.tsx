...
function TierTeamRow({ team, eventKey, ranking, categories, tags, overallRank, tierIndex, tierSize, canEdit, saving, dragging, dropTarget, onDragStart, onDragEnd, onDragOver, onDrop, onTierChange, onMove, onNote, onSelected, onTags }: { team: Team; eventKey: string; ranking?: Ranking; categories: Category[]; tags: PicklistTag[]; overallRank: number; tierIndex: number; tierSize: number; canEdit: boolean; saving: boolean; dragging: boolean; dropTarget: boolean; onDragStart: (event: DragEvent<HTMLElement>) => void; onDragEnd: () => void; onDragOver: (event: DragEvent<HTMLElement>) => void; onDrop: (event: DragEvent<HTMLElement>) => void; onTierChange: (categoryId: string) => void; onMove: (direction: -1 | 1) => void; onNote: (note: string) => void; onSelected: (selected: boolean) => void; onTags: (tagIds: string[]) => void }) {
  ...
  const scoutingMetrics = stats?.matches ? <dl className="picklist-scouting-metrics" aria-label={`Scouting data for Team ${team.team_number}`}><div><dt>Auto</dt><dd>Avg <b>{stats.autoFuel.toFixed(1)}</b> · Peak <b>{stats.autoPeakFuel.toFixed(1)}</b></dd></div><div><dt>Teleop</dt><dd>Avg <b>{stats.teleopFuel.toFixed(1)}</b> · Peak <b>{stats.teleopPeakFuel.toFixed(1)}</b></dd></div><div><dt>Broken</dt><dd><b>{stats.brokenPercent.toFixed(0)}%</b></dd></div></dl> : <span className="picklist-scouting-empty">No qualification/playoff scout data</span>;
